...
Generated by `generateCertificateId(userId, moduleId)` in `src/features/learning-module/utils/certificate-utils.ts`.

## Integration with Learning Modules

Starting from v2.0.0, certificates are integrated directly into the module detail page:
- **Completed Status**: The sidebar certificate preview changes to "Achievement" mode.
- **Dynamic ID**: The actual generated certificate ID is displayed on the preview.
- **Direct Buttons**: Users can click "Unduh Sertifikat" or "Lihat Versi Publik" directly from the module page.
- **Automatic Sync**: Certificate data is fetched in parallel with module progress for a seamless experience.
...
│   ├── utils/
│   │   └── certificate-utils.ts        # Pure utility functions (generateCertificateId)
│   └── types/
